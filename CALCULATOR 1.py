...
class calc: 
    ...
    def __init__(self,master): 
            ...
            master.geometry() 
            self.e = Entry(master) 
            self.e.grid(row=0,column=0,columnspan=20,pady=10) 
            self.e.focus_set() #Sets focus on the input text area 
  
            # Generating Buttons 
            Button(master,text="=",width=20,height=5, 
                   bg="red",command=lambda:self.equals()).grid( 
                                     row=4, column=4,columnspan=6) 
  
            Button(master,text='AC',width=9,height=5, 
                           bg="red", 
             command=lambda:self.clearall()).grid(row=1, column=4) 
  
            Button(master,text='C',width=9,height=5, 
                   bg="red", 
                   command=lambda:self.clear1()).grid(row=1, column=5) 
  
            Button(master,text="+",width=9,height=5, 
                   bg="red", 
                   command=lambda:self.action('+')).grid(row=4, column=3) 
  
            Button(master,text="x",width=9,height=5, 
                    bg="red", 
            ...
            Button(master,text="root",width=9,height=5, 
                   bg="red", 
                   command=lambda:self.squareroot()).grid(row=3, column=4) 
  
            Button(master,text="x^2",width=9,height=5, 
                   bg="red", 
